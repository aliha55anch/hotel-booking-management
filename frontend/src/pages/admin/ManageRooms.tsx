...
import { PlusIcon, PencilIcon, TrashIcon, BedIcon, UsersIcon } from '@/components/ui/icons'
import { getHotels } from '@/services/hotelService'
import { getRoomsByHotel, deleteRoom } from '@/services/roomService'
...
function RoomRow({ room, onEdit, onDelete }: RoomRowProps) {
  ...
  return (
    <tr className="hover:bg-surface/60">
      ...
      <td className="px-4 py-3 font-semibold text-primary">{formatPrice(room.pricePerNight, currency)}<span className="text-xs font-normal text-muted"> / night</span></td>
...
export default function ManageRooms() {
  ...
  useEffect(() => {
    ...
    getHotels({ limit: 100 })
      ...
      .catch((err) => {
        if (!cancelled) setError(getApiErrorMessage(err, 'Could not load hotels'))
      })
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Rooms" subtitle="Manage rooms for each hotel">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label>
          ...
          <select
            ...
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setSelectedHotelId(e.target.value)}
          >
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-4 rounded-card border border-line bg-surface p-12 text-center">
          ...
          <div>
            <h2 className="font-heading text-xl font-semibold text-ink">No hotels available</h2>
            <p className="mt-1 text-sm text-muted">Add a hotel first so you can manage its rooms.</p>
          </div>
          <Button to="/admin/hotels">Manage hotels</Button>
        </div>
      ) : rooms.length > 0 ? (
