...
import type { Hotel, Offer, Testimonial } from '@/types'
...
const SectionTitle = ({ title, subtitle, align }: SectionTitleProps) => (
  <div
...
function HeroSearch() {
  ...
  const SearchLabel = ({ icon, htmlFor, children }: SearchLabelProps) => (
    <label
      htmlFor={htmlFor}
    ...
    >
      ...
      <div className="sm:col-span-1 lg:col-span-2">
        ...
        <input
          ...
          max={4}
          value={guests}
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => setGuests(Math.max(1, Math.min(4, Number(e.target.value) || 1)))}
          type="number"
          className={searchFieldClass}
        />
      </div>
...
function RoomCard({ hotel, index }: RoomCardProps) {
      ...
      <div className="p-3 sm:p-4 pt-4 sm:pt-5">
        ...
        <div className="mt-3 sm:mt-4 flex items-center justify-between">
          <p className="text-sm sm:text-base">
            <span className="text-base sm:text-xl text-gray-800">{hotel.rating != null ? formatPrice((hotel as unknown as { priceFrom?: number }).priceFrom ?? 0, currency) : 'Rs ---'}</span>
