...
import { MapPinIcon, StarIcon, HotelIcon, ChevronLeftIcon, CalendarIcon, TrashIcon, FlameIcon } from '../components/ui/icons'
import { getHotelById } from '../services/hotelService'
import { getRoomsByHotel } from '../services/roomService'
import { getReviewsByHotel, createReview, deleteReview } from '../services/reviewService'
import { checkAvailability } from '../services/bookingService'
import { getApiErrorMessage } from '../lib/errors'
import { formatPrice } from '../lib/format'
import { useAuth } from '../context/AuthContext'
import { useCurrency } from '../context/CurrencyContext'
import { galleryFor } from '../lib/siteImages'
...
function BookingModal({ hotel, open, onClose, offerId, packageId }: BookingModalProps) {
...
function ReviewForm({ hotelId, onCreated }: ReviewFormProps) {
...
function ReviewList({ reviews, canDelete, onDelete, deletingId }: ReviewListProps) {
  if (!reviews.length) {
          ...
          <li key={review._id} className="rounded-card border border-line bg-background p-4">
            <div className="flex items-center gap-3">
              <span className="flex h-9 w-9 items-center justify-center rounded-full bg-primary-soft text-sm font-semibold text-primary">
                {(reviewUser?.name || 'R').charAt(0).toUpperCase()}
              </span>
              <div className="flex-1">
                <p className="text-sm font-semibold text-ink">{reviewUser?.name || 'Guest'}</p>
...
export default function HotelDetail() {
  ...
  const canDeleteReview = (review: Review) => {
    ...
    const reviewUser = typeof review.user === 'object' ? review.user : null
    if (reviewUser?._id && String(reviewUser._id) === myUserId) return true
