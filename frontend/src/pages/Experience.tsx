...
import { StarIcon, CalendarIcon, CheckIcon, ArrowRightIcon } from '../components/ui/icons'
...
export default function Experience() {
  ...
  const handleSubmitReview = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await createReview({ hotel: '', rating: reviewRating, comment: reviewText.trim() }, token)
      setReviewSuccess(true)
      setReviewText('')
      setReviewRating(5)
    } catch (err: unknown) {
      const axiosErr = err as { response?: { data?: { message?: string } } }
      setReviewError(axiosErr.response?.data?.message || 'Failed to submit review. Please try again.')
    } finally {
      setSubmitting(false)
    }
  }
