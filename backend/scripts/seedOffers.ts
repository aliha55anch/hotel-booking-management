...
import { findAccountByEmail } from '../services/userAccountService'

const getHotelId = async (name: string) => {
  const hotel = await Hotel.findOne({ name })
  return hotel ? hotel._id : null
}

interface PackageOptionSeed {
  name: string
  nights: number
  description: string
  price: number
...
interface OfferSeed {
  ...
  expiryDate: Date
  highlights: string[]
  packageOptions: PackageOptionSeed[]
  active: boolean
}
...
const run = async (): Promise<void> => {
  ...
  for (const data of offers) {
    ...
    const offer = await Offer.create({ ...data, packageOptions, owner, ownerModel } as any)

    console.log(
      `SEEDED: ${offer.title} | discount: ${offer.discountPercent}% | expiry: ${offer.expiryDate?.toISOString().slice(0, 10) ?? 'no expiry'} | packages: ${packageOptions.length}`
    )
  }
