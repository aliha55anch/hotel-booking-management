...
import { BedIcon, CalendarIcon, CheckIcon, FlameIcon } from '../ui/icons'
...
export default function BookingCard({ booking, onCancel, cancelling }: BookingCardProps) {
  ...
  const hotelName = typeof booking.hotel === 'object' ? (booking.hotel as Hotel).name : 'Hotel'
  const roomType = typeof booking.room === 'object' ? (booking.room as Room).roomType : 'Room'
  const offerTitle = typeof booking.offer === 'object' ? (booking.offer as Offer).title : undefined
    ...
    <div className="flex flex-col gap-4 rounded-card border border-line bg-background p-4 shadow-card sm:flex-row sm:p-5">
      ...
      <div className="flex flex-1 flex-col gap-2">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h3 className="font-heading text-lg font-semibold text-ink">{hotelName}</h3>
            <p className="mt-0.5 text-sm text-muted">{roomType}</p>
          </div>
