...
const roomImg = (name: string): string => `/images/${name}`
...
const run = async (): Promise<void> => {
  ...
  for (const data of hotels) {
    ...
    const hotel = await Hotel.create({ ...hotelData, owner, ownerModel } as any)

    for (const room of rooms) {
      await Room.create({ hotel: hotel._id, ...room })
    }

    console.log(`SEEDED: ${hotel.name} | city: ${hotel.city} | rating: ${hotel.rating} | images: ${hotel.images?.length ?? 0} | rooms: ${rooms.length}`)
  }
