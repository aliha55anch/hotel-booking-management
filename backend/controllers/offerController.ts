...
const getOffers = asyncHandler(async (req: Request, res: Response) => {
...
const getAllOffers = asyncHandler(async (req: Request, res: Response) => {
  const user = await getRequestUser(req)

  if (!isStaff(user?.role as string)) {
    res.status(403)
...
const getOfferById = asyncHandler(async (req: Request, res: Response) => {
  const offer = await Offer.findById(req.params.id as string)
...
const createOffer = asyncHandler(async (req: Request, res: Response) => {
  const user = await getRequestUser(req)
  ...
  const offer = await Offer.create({
    ...
    active,
    owner: user._id,
    ownerModel: user.userModel as 'Owner' | 'Admin',
  })

  res.status(201).json({ success: true, offer })
})

const updateOffer = asyncHandler(async (req: Request, res: Response) => {
  const offer = await Offer.findById(req.params.id as string)
...
const deleteOffer = asyncHandler(async (req: Request, res: Response) => {
  const offer = await Offer.findById(req.params.id as string)
  ...
  await Offer.findByIdAndDelete(req.params.id as string)

  res.status(200).json({ success: true, message: 'Offer deleted' })
})

export { getOffers, getAllOffers, getOfferById, createOffer, updateOffer, deleteOffer }
