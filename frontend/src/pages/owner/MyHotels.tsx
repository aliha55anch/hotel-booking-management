...
import { PlusIcon, PencilIcon, TrashIcon, HotelIcon, StarIcon, BedIcon, MapPinIcon } from '@/components/ui/icons'
...
function MyHotelRow({ hotel, roomCount, onEdit, onDelete }: MyHotelRowProps) {
  return (
    ...
    getMyHotels(token!)
      .then(async (data) => {
        ...
        const counts = await Promise.all(
          hotelList.map(async (hotel: Hotel) => {
            try {
              const rooms = await getRoomsByHotel(hotel._id)
              return [hotel._id, rooms.count || rooms.rooms?.length || 0] as const
            } catch {
              return [hotel._id, 0] as const
            }
          })
        )
