...
const updateHotelRating = async (hotelId: string): Promise<number> => {
...
const createReview = asyncHandler(async (req: Request, res: Response) => {
  ...
  let review
  try {
    review = await Review.create({ user: user._id, userModel: user.userModel as 'User' | 'Owner' | 'Admin', hotel, rating, comment })
  } catch (error: any) {
    if (error.code === 11000) {
      res.status(400)
...
const getReviewsByHotel = asyncHandler(async (req: Request, res: Response) => {
...
export { createReview, getReviewsByHotel, deleteReview }
