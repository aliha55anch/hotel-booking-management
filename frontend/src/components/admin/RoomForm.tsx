...
import { inputClass, textareaClass, labelClass } from './formClasses'
...
const toValues = (room: Room): RoomFormValues => ({
  hotel: typeof room.hotel === 'object' ? (room.hotel as unknown as { _id: string })._id : room.hotel || '',
...
export default function RoomForm({ open, room, hotels, token, onClose, onSaved }: RoomFormProps) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<RoomFormValues>({ defaultValues: emptyValues })
