...
import { getStats } from '../services/hotelService'
import {
  HotelIcon,
  CalendarIcon,
  CheckIcon,
...
export default function About() {
  ...
  useEffect(() => {
    ...
    getStats()
      .then((data) => {
        if (!cancelled) setStats({ hotels: data.totalHotels, bookings: data.totalBookings, rating: data.totalRevenue > 0 ? data.totalRevenue : undefined })
      })
      .catch(() => {})
    return () => {
      cancelled = true
