...
import { createPaymentIntent, getExchangeRate } from '../services/stripeService'
...
const runCheckout = async ({ roomId, checkIn, checkOut, token, stripe, elements }: RunCheckoutParams) => {
...
function CheckoutLayout({ hotel, rooms, checkIn, checkOut, guests, initialRoomId, onSubmit, status, error, showCard, stripeReady }: CheckoutLayoutProps) {
  ...
  const nights = Math.max(1, Math.round((checkOutDate.getTime() - checkInDate.getTime()) / MS_PER_DAY))
...
function SimpleCheckout(props: StripeCheckoutProps) {
  const { token } = useAuth()
  const navigate = useNavigate()
  const [status, setStatus] = useState<'idle' | 'submitting' | 'error'>('idle')
  const [error, setError] = useState<string | null>(null)
  ...
  return <CheckoutLayout {...props} onSubmit={handleSubmit} status={status} error={error} showCard={false} stripeReady={false} />
}

function PaymentCheckout(props: StripeCheckoutProps) {
