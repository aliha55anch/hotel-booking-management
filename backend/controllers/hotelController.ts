...
import { findAccountById, updateAccount } from '../services/userAccountService'
...
const canManageHotel = (hotel: any, user: any): boolean => {
...
const getAllHotels = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (hotels.length > 0) {
    ...
    if (minPriceNum != null || maxPriceNum != null) {
      roomFilter.pricePerNight = {} as Record<string, number>
      if (minPriceNum != null) (roomFilter.pricePerNight as Record<string, number>).$gte = minPriceNum
      if (maxPriceNum != null) (roomFilter.pricePerNight as Record<string, number>).$lte = maxPriceNum
    }
...
const getHotelById = asyncHandler(async (req: Request, res: Response) => {
  const hotel = await Hotel.findById(req.params.id as string)
...
const createHotel = asyncHandler(async (req: Request, res: Response) => {
  ...
  const ownerModel = user.role === 'user' ? 'Owner' : user.userModel as 'Owner' | 'Admin'
...
const deleteHotel = asyncHandler(async (req: Request, res: Response) => {
  ...
  await Hotel.findByIdAndDelete(req.params.id as string)
  await Room.deleteMany({ hotel: req.params.id as string })

  res.status(200).json({ success: true, message: 'Hotel deleted' })
})

export { getAllHotels, getStats, getHotelById, getMyHotels, createHotel, updateHotel, deleteHotel }
